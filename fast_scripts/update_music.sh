...
update_playlist() {
	...
	yt-dlp $url -x --audio-format mp3 --download-archive $HOME/Music/$name/songs.txt -o "$HOME/Music/$name/%(title)s.%(ext)s" --postprocessor-args "-map_metadata -1 -metadata Album='$name' -metadata album='$name' -metadata artist='xqweak'"
	echo "done"
}
